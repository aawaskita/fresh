...
def temp(zeit):
    """
    fuel element temperature calculation
    """

    zeit0h = globalVar.real1['zeit0d'] * 24.		  # zeit0d unit is DAY: time of normal operation, zeit0h unit in HOURs
    zeit0  = zeit0h * 3600.
    zeit1  = zeit / 3600.				   #input dalam [seconds] diubah ke [hours]
    
    print(' normal operation time: ' + str(globalVar.real1['zeit0d']) + 'days')
    print(' normal operation time: %.3f hours' % zeit0h)
    print(' normal operation time: %.3f seconds' % zeit0)
    print(' time input: ' + str(zeit1))
    
    if zeit1>zeit0h:						# after the normal condition --> accident condt.
	print('---Kasus Accident Condition atau Heating---')
	x=globalVar.itemp['ni']
	for i in range(x):
	    n = i
	    temp = globalVar.ttemp['atemp'][i]
	    ze   = globalVar.ttemp['azeit'][i] + zeit0h
	    if zeit1<=ze:
		print(' input waktu kurang atau sama dengan data ke-',i , '= ', ze, 'hours')
		if n!=1 and n<=x:
		    at2 = globalVar.ttemp['atemp'][n]
		    at1 = globalVar.ttemp['atemp'][n-1]
		    az2 = globalVar.ttemp['azeit'][n] + zeit0h
		    az1 = globalVar.ttemp['azeit'][n-1] + zeit0h
		    temp= polat(zeit1,az1,az2,at1,at2)
		    print(' Temp. pada waktu input: '+str(temp) + 'deg.C ='  +str(temp+273.) +'K')
		    break
    else:											   #still in normal operation
	print('---Kasus Normal Operation atau Irradiation---')
	x=globalVar.itemp['ni0']
	for i in range(x):
	    n = i
	    temp = globalVar.ttemp['atemp0'][i]
	    ze   = globalVar.ttemp['azeit0'][i] * 24.	 
	    if zeit1<=ze:
		print(' input waktu kurang atau sama dengan data ke-',i , '= ', ze, 'hours')
		if n!=0 and n<=x:
		    at2 = globalVar.ttemp['atemp0'][n]
		    at1 = globalVar.ttemp['atemp0'][n-1]
		    az2 = globalVar.ttemp['azeit0'][n] * 24.
		    az1 = globalVar.ttemp['azeit0'][n-1] * 24.
		    temp= polat(zeit1,az1,az2,at1,at2)
		    print(' Temp. pada waktu input: ' +str(temp) + 'deg.C =' + str(temp+273.) + 'K')
		    break
    temp = temp + 273.
...
def kudif(n,r, dti, uez, di, q, zerfk, t0, t, ges):	 #being updated
    ...
    DC/DT=DI*(D2C/DR**2+2/R*DC/DR)+Q-ZERFK*C IN THE RANGE R(1) TO R(N+1)
    BOUNDARY CONDITION: R=R(n+1)  DI*DC/DT=UEZ*(C-T0) R=R(1)	DC/DR=0
...
def instat():
	...
	"""
	cgm is the average concentration in Fe-graphite, is used as boundary condition for difusion in CP
	"""
	cgm=0.0
	...
	"""
	ifstop=1, as soon as fuel element release has reached 100%
	"""
	ifstop=0
	...
	globalVar.zeit2['zeit'] = globalVar.zeit2['zeit'] + globalVar.zeit2['dzeit']
